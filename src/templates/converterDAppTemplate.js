...
function createTemplate(projectPath) {
  const mainFilePath = path.join(projectPath, 'converter.py');
...
`);
  fs.writeFileSync(readmeFilePath, "# Converter dApp\nThis is a template for a Converter decentralized application.");
  fs.writeFileSync(dockerbakeFilePath, 
...
services:
  server_manager:
    image: \${DAPP_IMAGE:-cartesi/dapp:converter-devel-server}
`
  );
